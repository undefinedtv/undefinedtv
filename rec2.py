                ...
                if 'sources' in content and isinstance(content['sources'], list):
                    for source_item in content['sources']:
                        if source_item.get('type') == 'm3u8' and 'url' in source_item:
                            ...
                            if categories != "Spor" or re.search(title, 'S Sport', re.IGNORECASE)  or re.search(title, 'TABİİ SPOR', re.IGNORECASE)  or re.search(title, 'Bein Sports', re.IGNORECASE):
                                continue
...
def main():
    ...
    # 5. Dosyaya kaydet
    with open('rec.m3u', 'w', encoding='utf-8') as f:
        f.write(m3u_content)
